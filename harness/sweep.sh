...
capture_backend() {
  ...
  if command -v xdpdump >/dev/null 2>&1; then
    timeout 5 xdpdump -i "$IFACE" -w "$out" --rx-cpu-mode=smp &
    local xdppid=$!
    sleep 0.5
    ...
    echo "xdpdump not found; writing inject-only stub manifest inputs" >&2
    ip netns exec "$NS_INJ" python3 harness/inject.py corpus/corpus.pcap "$INJ_IFACE"
    touch "$out"
  fi
  detach_xdp
...
python3 - "$MANIFEST" "$PROFILE" "$COMMIT" <<'PY'
...
try:
    out = subprocess.check_output(["ethtool", "-i", sys.argv[4] if len(sys.argv) > 4 else "veth-b"], text=True)
